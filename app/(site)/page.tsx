...
async function getTwelveHourForecast(): Promise<HourlyForecast[]> {
  ...
  try {
    ...
    if (!res.ok) {
      if (process.env.NODE_ENV === 'development') {
        console.warn(
          'Weather (12hr): failed to fetch forecast',
          res.status,
          res.statusText,
        );
      }
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 space-y-8 md:space-y-10">
        <HeroSection nextEvent={nextEvent} latestPost={latestPost} />
...
function HeroSection({
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-700 text-white shadow-[0_28px_70px_rgba(15,118,110,0.6)] border border-emerald-900/40">
      {/* Background photo overlay (decorative) */}
      ...
      <div className="relative px-6 py-10 md:px-10 md:py-16 flex flex-col md:flex-row md:items-center md:justify-between gap-8">
          ...
          <div className="flex flex-col gap-3 pt-1">
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                ...
                aria-label="View community events calendar"
              >
              ...
              <Link
                href="/news"
                className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200/80 bg-emerald-900/40 text-emerald-50 text-sm font-medium px-4 py-2 hover:bg-emerald-800/70 hover:-translate-y-[1px] transition"
                aria-label="View all community news"
              >
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-emerald-50/90">
              <Link
                href="/pool"
        ...
        {/* Highlight card */}
        <HeroHighlightCard nextEvent={nextEvent} latestPost={latestPost} />
      </div>
    </section>
  );
}
      ...
      {nextEvent ? (
        <>
          ...
          <Link
            ...
            aria-label="View full events calendar for Cypressdale"
          >
            View full calendar →
          </Link>

          {latestPost && (
            <LatestNewsTeaser latestPost={latestPost} showBorder />
          )}
        </>
      ) : (
        <>
...
function LatestNewsTeaser({
  ...
  return (
    <div
      className={`mt-4 ${
        showBorder ? 'border-t border-emerald-100 pt-3' : ''
      }`}
    >
      ...
      <Link
        ...
        aria-label="Read all Cypressdale news"
      >
        Read all news →
      </Link>
    </div>
...
function SiteNotice() {
  return (
    <section
      className="rounded-2xl border border-amber-200/90 bg-white/95 text-amber-900 flex gap-3 items-start text-sm shadow-[0_16px_40px_rgba(15,118,110,0.18)] backdrop-blur px-4 py-3 md:px-5 md:py-4"
      role="status"
      aria-live="polite"
    >
...
function WeatherSection({
  ...
  today?: DailyForecast;
  otherDays: DailyForecast[];
  hourlyForecast: HourlyForecast[];
}) {
  const hasForecast = !!today;
  ...
  return (
    <section className="px-3 sm:px-4 py-5 md:px-6 md:py-6 space-y-3 flex flex-col items-center text-center">
      ...
      <p className="muted text-sm">
        5-day and 12-hour forecast for Cypressdale. Data provided by
        AccuWeather.
      </p>
...
function LatestNewsSection({ posts }: { posts: Post[] }) {
  return (
    <section className="rounded-3xl bg-white/95 border border-emerald-50 shadow-[0_18px_50px_rgba(15,118,110,0.22)] backdrop-blur px-4 py-5 md:px-6 md:py-6 space-y-3">
      <div className="flex items-baseline justify-between gap-2">
...
function AboutSection() {
  return (
    <section className="rounded-3xl bg-white/95 border border-emerald-50 shadow-[0_18px_50px_rgba(15,118,110,0.22)] backdrop-blur px-4 py-5 md:px-6 md:py-6 space-y-5 text-center">
      {/* Text content */}
      <div className="space-y-3">
        <h2 className="h2 mb-1">About Cypressdale</h2>
